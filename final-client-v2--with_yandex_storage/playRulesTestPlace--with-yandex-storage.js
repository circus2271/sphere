...
const projectFolderName = 'sphere-test-bucket'
const placeUniqueName = 'test-place';
...
const placeFolderName = 'test-place'

const playlists = {
  'Morning list': 'test_playlist.txt',
  'Day list': 'test_playlist.txt'
}
...
$.ajax({
  ...
  // url: `${window['objectStorageUrl']}/${projectFolderName}/${placeFolderName}/${playlists['Morning list']}`,
  url: `${window['objectStorageUrl']}/${placeFolderName}/${playlists['Morning list']}`,
  ...
  success: function (data) {
    ...
    for (var i = 0; i < list1.length; i++) {
      list1[i] = `${window['musicLibraryUrl']}/${list1[i]}`;
    }
    ...
    $.ajax({
      // url: `${window['objectStorageUrl']}/${projectFolderName}/${placeFolderName}/${playlists['Day list']}`,
      url: `${window['objectStorageUrl']}/${placeFolderName}/${playlists['Day list']}`,
      contentType: "text/plain; charset=utf-8",
      success: function (data) {
        // -----======================================================CAHNGE HERE restaurant path=========================================----
        ...
        for (var i = 0; i < list2.length; i++) {
          list2[i] = `${window['musicLibraryUrl']}/${list2[i]}`;
        }
...
function insertingLikeDislikeFormData() {
  ...
  if (sound1.playing() && !sound2.playing()) {
    ...
    // replace is used to remove "path-prefix" from url:
    // https://sphere-test-bucket.website.yandexcloud.net/test-foldre/songname.mp3 -> songname.mp3
    var song1name = sound1._src.replace(/.*[/]/ig, '');
    $("input[name=songname]").val(song1name);
  } else if (!sound1.playing() && sound2.playing()) {
    //console.log(sound2._src.split("musicLibrary/")[1]);
    var song2name = sound2._src.replace(/.*[/]/ig, '');
